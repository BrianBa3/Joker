...
class TicketData {
	...
	public function getUsNam(){ return UserData::getById($this->user_id); }
	public function getTicketName() { return Inc_NombreTicket::getById($this->title); }
...
// partiendo de que ya tenemos creado un objecto TicketData previamente utilizamos el contexto
	...
	public static function getAllMy($usr){
		$sql = "select * from ".self::$tablename." where user_id=\"$usr\"";
		$query = Executor::doit($sql);
		return Model::many($query[0],new TicketData());
	}
	...
	public static function getAllMyPendings($usr){
		$sql = "select * from ".self::$tablename." where status_id=1 and user_id=\"$usr\"";
		$query = Executor::doit($sql);
		return Model::many($query[0],new TicketData());
	}
	...
	public static function getAllMyDesarrollo($usr){
		$sql = "select * from ".self::$tablename." where status_id=2 and user_id=\"$usr\"";
		$query = Executor::doit($sql);
		return Model::many($query[0],new TicketData());
	}
	...
	public static function getAllMyDetenidos($usr){
		$sql = "select * from ".self::$tablename." where status_id=5 and user_id=\"$usr\"";
		$query = Executor::doit($sql);
		return Model::many($query[0],new TicketData());
	}
